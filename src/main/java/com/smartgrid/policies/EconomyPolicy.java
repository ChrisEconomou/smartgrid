...
public class EconomyPolicy implements HouseholdPolicy {
	...
	public EconomyPolicy(String iD,String author,String version,Double maxEP,Date simulationSD) {
		super();
		this.setID(iD);
...
//	 */
	...
	public void tick(int tickValue, int granularity, double electricityPrice) {
        ...
        try {
			startDate = parser.parse("07:00");
			endDate = parser.parse("17:00");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
       
       //if (time=7:00-17:00 then close Appliance
        if( UtilityMethods.isTimeBetween(currentSimulationDate.getTime(), startDate.getTime(), endDate.getTime())){
        	
        	System.out.println("The time is between 07:00 and 17:00 close the fucking appliances");
       ...
       if(UtilityMethods.isItWeekend(simulationStartDate, currentTimeinMin) && electricityPrice<=getIdealElectricityPrice()){
